...
open class BarChartRenderer: BarLineScatterCandleBubbleRenderer
{
	...
	open func drawBar(context: CGContext, dataSet: IBarChartDataSet, index: Int, bezierPath: UIBezierPath)
	{
		context.saveGState()
		...
		{
			drawGradient(context: context, bezierPath: bezierPath, gradientColors: gradientColor, orientation: dataSet.barGradientOrientation)
		}
		else
		{
			// Set the color for the currently drawn value. If the index is out of bounds, reuse colors.
	...
	open func drawGradient(context: CGContext, bezierPath: UIBezierPath, gradientColors: Array<NSUIColor>, orientation: BarGradientOrientation)
	{
    ...
    open override func drawHighlighted(context: CGContext, indices: [Highlight])
    {
        ...
        for high in indices
        {
            ...
            if let e = set.entryForXValue(high.x, closestToY: high.y) as? BarChartDataEntry
            {
				...
				if dataProvider.isDrawRoundedBarEnabled
				{
					let cornerRadius = set.barCornerRadius > 0 ? CGSize(width: set.barCornerRadius, height: set.barCornerRadius) : CGSize(width: barRect.width / 2.0, height: barRect.width / 2.0)
					#if os(OSX)
					let bezierPath = NSBezierPath(roundedRect: barRect, xRadius: cornerRadius.width, yRadius: cornerRadius.height)
					context.addPath(bezierPath.cgPath)
					#else
